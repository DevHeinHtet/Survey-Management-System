...
<div class="p-8">
    <div class="flex items-center space-x-4">
        <span class="text-3xl font-bold">{{$manager->name}}</span>
        @if ($manager->detail && $manager->detail->url != null)
            <a href="{{ $manager->detail->url }}" class="text-blue-600 hover:underline text-sm cursor-pointer">website</a>
        @endif
    </div>
    <div class="flex justify-between items-center md:mr-20">
        <div class="flex flex-col md:flex-row items-center py-8 space-x-4">
            <div class="relative">
                <img src="{{$manager->getProfile()}}" class="w-28 h-28 object-cover rounded-full ring-1 ring-offset-2 ring-blue-500 mb-2" alt="Logo">
                <label for="profile">
                ...
                <form action="{{ route('manager.updatePhoto',['id' => $manager->hash]) }}" id="upload" method="post" enctype="multipart/form-data">
                    @csrf
            ...
            <div class="flex flex-col">
                ...
                <a onclick="openReset()" class="text-blue-600 hover:underline text-sm cursor-pointer">Reset password</a>
            </div>
        </div>
    ...
    <form id="profile-data-form" action="{{ route('manager.updateInfo',['id' => $manager->hash]) }}" method="POST">
        @csrf
        @if(count($errors) > 0)
        ...
        <div class="grid grid-cols-1 gap-3 mb-6 md:grid-cols-2 md:mr-20">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ $manager->name }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 block w-full py-2" placeholder="Enter name" required>
            </div>
            <div>
                <label for="email" class="block mb-2 text-sm font-medium text-gray-900">Email address</label>
                <input type="email" name="email" id="email" value="{{ $manager->email }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 block w-full py-2" placeholder="Enter email" required>
            </div>
            <div>
...
<script>
    ...
    function openReset(){
        ...
        link.href = "{{ url("/manager/reset-password/$manager->hash") }}";
